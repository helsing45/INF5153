...
public class Main extends JFrame {
	...
	private OperatorLabel labelAnimal;
	...
	public Main() {
		...
		labelAnimal = new OperatorLabel(new And());
	...
	class TransferableAnimal implements Transferable {
		...
		private Operator animal;

		public TransferableAnimal(Operator ani) {
			this.animal = ani;
		}
	...
	class DragGestureListImp implements DragGestureListener {
		...
		public void dragGestureRecognized(DragGestureEvent event) {
			Cursor cursor = null;
			OperatorLabel lblAnimal = (OperatorLabel) event.getComponent();

			if (event.getDragAction() == DnDConstants.ACTION_COPY) {
				cursor = DragSource.DefaultCopyDrop;
			}
			Operator animal = lblAnimal.getOperator();
	...
	class MyDropTargetListImp extends DropTargetAdapter implements
		...
		public void drop(DropTargetDropEvent event) {
			try {
				...
				Operator an = (Operator) tr.getTransferData(dataFlavor);

				if (event.isDataFlavorSupported(dataFlavor)) {
					event.acceptDrop(DnDConstants.ACTION_COPY);
					this.panel.add(new OperatorLabel(an));
